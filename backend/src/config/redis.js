...
const redisConfig = {
    ...
    // Max Connections
    maxRetriesPerRequest: 1,        // Số lần retry tối đa cho mỗi request
    maxLoadingRetryTime: 1000,     // Thời gian tối đa retry khi loading (ms)

    // Connection Timeout
    connectTimeout: 3000,          // Timeout khi connecting (ms)
    commandTimeout: 1000,           // Timeout cho mỗi command (ms)

    // Retry Strategy
    retryStrategy(times) {
        return delay = Math.min(times * 50, 1000); // Exponential backoff
    },
    ...
    // Pool Settings
    db: 0,                         // Database index
    maxConnections: 50,            // Số connections tối đa trong pool
    minConnections: 10,             // Số connections tối thiểu trong pool
